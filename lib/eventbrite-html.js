...
const fetch = async (url, qs = '') => {
  try {
    ...
    $list.each(function () {
      const $this = $(this);
      const title = $this.find('.eds-is-hidden-accessible').text();
      ...
      const rightDate = Date.parse(date.replace('Tomorrow at', tomorrowString));
  ...
  return new Promise(resolve => {
    fetch(config.url).then(events => {
      events = events.filter(event => {
        ...
        if (config.exclude instanceof Array && config.exclude.indexOf(event.link) !== -1) {
          return false;
        }
